<div class="bg-white rounded-lg border p-6 mb-5">
    <div class="flex flex-col lg:flex-row gap-4">
        ...
        <div class="w-full lg:w-5/12">
            <img class="h-full" src="{{ url($cart_item['activity_image']??'storage/uploads/placeholder_image.png') }}" alt="Desert Tour Image" class="rounded-lg object-cover">
        </div>
        <!-- Content -->
            ...
            <div class="mb-2 flex items-center">
                ...
                @if(isset($cart_item['category']))
                    @if($cart_item['category'] === 'private')
                        <p>Groups: <span class="text-sm text-gray-600">{{ $cart_item['group'] }}</span></p>
                        <p>Persons per group: <span class="text-sm text-gray-600">{{ $cart_item['group_size'] }}</span></p>
                    @else
                        <p>Adult: <span class="text-sm text-gray-600 mr-2">{{ $cart_item['adult'] }}</span></p>
                        <p>Child: <span class="text-sm text-gray-600 mr-2">{{ $cart_item['child'] }}</span></p>
                        <p>Infant: <span class="text-sm text-gray-600 mr-2">{{ $cart_item['infant'] }}</span></p>
                    @endif
                    <span class="text-sm text-gray-600">({{ $cart_item['category'] }})</span>
                @endif
            </div>
