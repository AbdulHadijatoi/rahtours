<li class="mb-5">
    <div class="flex items-center p-4 shadow-md rounded-none hover:bg-gray-100 ">
        <a href="{{ url('dubai-activities/'.$activity->category->slug . '/' . $activity->slug) }}">
            <img src="{{ url($activity->image_url) }}" class="w-[12rem] min-h-[12rem] rounded object-cover mr-4">
        </a>
        <div class="flex flex-col justify-start items-between">
            ...
            <a href="{{ url('remove-from-wishlist/'. $activity->id) }}" class="btn btn-sm w-auto remove-from-wishlist">Remove from wishlist</a>
        </div>
    </div>
</li>